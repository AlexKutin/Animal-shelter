...
ALTER TABLE report_cat_shelter ADD COLUMN IF NOT EXISTS file_path VARCHAR(255);

-- Добавление столбца file_size для хранения размера файла изображения
ALTER TABLE report_cat_shelter ADD COLUMN IF NOT EXISTS file_size int8;

-- Добавление столбца media_type для хранения типа файла изображения
ALTER TABLE report_cat_shelter ADD COLUMN IF NOT EXISTS media_type VARCHAR(25);

-- changeset AlexKutin:7
-- Удаление лишних столбцов
ALTER TABLE report_cat_shelter DROP COLUMN IF EXISTS photo_data;

ALTER TABLE report_cat_shelter DROP COLUMN IF EXISTS photo_filename;
